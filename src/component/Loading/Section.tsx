...
const LoadingSection = () => {
  ...
  return (
    <section className={`w-[100svw] h-[100svh] flex justify-center items-center fixed top-0 left-0 z-50 overflow-hidden bg-[${isLoading ? "white" : "none"}]`}>
        <div className={`panel w-full h-full bg-[white] overflow-hidden flex items-center justify-end transition-all duration-1000 translate-x-[${isLoading ? "0%" : "-100%"}]`}>
        ...
        <div className={`panel w-full h-full bg-[white] overflow-hidden flex items-center justify-start transition-all duration-1000 translate-x-[${isLoading ? "0%" : "100%"}]`}>
        ...
        <div className={`absolute bottom-[20svh] overflow-hidden`}>
            <LoadingButton onClick={openPanel} className={`relative duration-500 transition-all translate opacity-${button ? "100" : "0"}`}/>
        </div>
    </section>
  )
}
